...
Proxmox makes it quite easy to upload ISO files to the server. Simply click on the `local` storage on the left hand side, then click on `Content` and then click on `Upload`.
![prox2](./prox2.png)
